...
export default function ProductCatalog({ onNavigateToContact }: ProductCatalogProps) {
  ...
  const categories = [
    { id: 'all', label: t('catalog_cat_all') },
    { id: 'interior', label: t('catalog_cat_interior') },
    { id: 'exterior', label: t('catalog_cat_exterior') }
  ];
  ...
  const subCategoryLabels: Record<string, string> = {
    'DÉCO': t('catalog_subcat_deco'),
    'IMPRESSION': t('catalog_subcat_impression'),
    'ENDUIT': t('catalog_subcat_enduit'),
    'ACRYLIQUE': t('catalog_subcat_acrylique'),
    'VERNIS': t('catalog_subcat_vernis'),
    'HYGIÈNE': t('catalog_subcat_hygiene'),
    'SOLUTIONS': t('catalog_subcat_solutions'),
  };
    ...
    <section className="py-20 bg-[#fafafa] dark:bg-[#111113] transition-colors duration-300" id="products-catalog-section">
      <div className="max-w-7xl mx-auto px-6 md:px-12">
        ...
        <div className="text-center max-w-3xl mx-auto mb-16">
          <div className="text-xs font-bold uppercase tracking-[0.2em] text-green-600 dark:text-green-400 mb-3">{t('catalog_badge')}</div>
          <h2 className="font-serif text-4xl md:text-5xl text-gray-900 dark:text-white leading-tight">
            {t('catalog_title_l1')} <br />{t('catalog_title_l2')}
          </h2>
        ...
        <div className="flex flex-wrap justify-center gap-2.5 mb-12 max-w-5xl mx-auto" id="subcategory-filters">
          {subCategories.map(sub => {
            const count = getSubCategoryCount(sub);
            if (count === 0 && sub !== 'all') return null; // hide empty subcategories for cleanliness
            
            const label = sub === 'all' ? t('catalog_subcat_all') : (subCategoryLabels[sub] ?? sub);
            return (
              <button
                key={sub}
                onClick={() => setActiveSubCategory(sub)}
                className={`px-5 py-2.5 rounded-lg text-xs md:text-sm font-bold uppercase tracking-wider transition-all cursor-pointer flex items-center gap-2 ${
          ...
          {filteredProducts.map((p, index) => (
            ...
            >
              ...
              <div>
               ...
               <div className="text-gray-400 dark:text-gray-500 text-[9px] uppercase font-bold tracking-widest">{p.line[lang]}</div>
               ...
               <p className="text-gray-500 dark:text-gray-400 font-sans text-xs leading-relaxed mb-4 font-light">{p.description[lang]}</p>
              </div>
